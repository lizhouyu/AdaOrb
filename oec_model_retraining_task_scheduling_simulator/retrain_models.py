...
def retrain_cloud_detection_model(
    ...
    output_channel_file_path = os.path.join(checkpoint_folder, 'best_channels.txt')
    if not os.path.exists(output_channel_file_path):
    ...
    # evaluate the model
    model.eval()
    dice_mean, dice_std, entropy_mean, entropy_std = eval_UNet(model, validation_dataset, device)
