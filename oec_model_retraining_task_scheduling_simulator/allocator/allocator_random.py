...
class RandomAllocator(object):
    def __init__(self, inputs, log_folder='logs'):
        ...
        self.logger = getMyLogger(os.path.join(log_folder, 'log_random_allocator.log'))

        # initialize random seed for reproducibility
        np.random.seed(self.random_seed)

        self.logger.info(f"RandomAllocator initialized with random seed: {self.random_seed}")
    ...
    def make_downlink_decision(self, time_step, v_sequence, b_sequence, state_vol, state_down, state_acc):
        ...
        # initialize the allocation decision
        allocation = [0] * len(v_current)
        # get a dictionary with keys as all the indices of non-zero state_vol and values are the number images to be downlinked
        dict_downlink_task_idx_num_downlink_images = {idx:  0 for idx, num_images in enumerate(state_vol) if num_images > 0}        
        for idx in range(len(v_current)):
            allocation[idx] = dict_downlink_task_idx_num_downlink_images[idx] if idx in dict_downlink_task_idx_num_downlink_images else 0
        # Randomly allocate bandwidth b_current to all the onboard tasks
        allocation = np.random.rand(len(v_current))
        allocation /= np.sum(allocation)
        allocation *= b_current
