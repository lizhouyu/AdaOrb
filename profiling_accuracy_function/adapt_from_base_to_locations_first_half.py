...
def adapt_from_base_to_location(location):
    dataset_root_dir = 'location_bootstrap_datasets'
    model_weight_path = os.path.join('checkpoints', 'pruned', 'building1', 'best.pt') #f"building/v8n_{location_A}/weights/best.pt"
    model_channel_filepath = os.path.join('checkpoints', 'pruned', 'building1', 'best_channels.txt')
    # model_weight_path = os.path.join('checkpoints', 'original', 'building1', 'best.pt') #f"building/v8n_{location_A}/weights/best.pt"
    # model_channel_filepath = os.path.join('checkpoints', 'original', 'building1', 'best_channels.txt') 
    model_input_size = (64, 64)
    training_image_pool_folder = os.path.join(dataset_root_dir, location, 'train', 'images')
    traning_label_folder = os.path.join(dataset_root_dir, location, 'train', 'masks')
    training_cache_folder = os.path.join('training_cache', 'building', f'adapt_from_base_to_{location}')

    # leave if the train cache folder already exists
    if os.path.exists(training_cache_folder):
        print(f'Training cache folder already exists: {training_cache_folder}')
        return

    val_image_folder = os.path.join(dataset_root_dir, location, 'val', 'images')
    val_label_folder = os.path.join(dataset_root_dir, location, 'val', 'masks')
    ...
    for round_idx in range(1, number_of_round + 1):
        ...
        for sampled_image_path in sampled_image_path_list:
            ...
            if np.random.rand() > 0.8 or is_first_image: # 20% for validation or the first image (make sure there is at least one image for validation) 
...
if __name__ == '__main__':
    dataset_root_dir = 'location_bootstrap_datasets'
    location_list = os.listdir(dataset_root_dir)
    location_list.sort()
    location_list = location_list[:len(location_list)//2]
    for location in location_list:
        adapt_from_base_to_location(location)
