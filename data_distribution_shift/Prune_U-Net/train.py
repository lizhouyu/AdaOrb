...
def train_net(net,
              location = 'mask64_AOI_0_global_sampled_5_fixed_Train',
              epochs=5,
              batch_size=1,
              lr=0.1,
              val_percent=0.05,
    ...
    dir_checkpoint = f'checkpoints/original/{location}/'
    os.makedirs(dir_checkpoint, exist_ok=True)

    train_dataset = UNetDataset(im_folder_path=train_im_folder_path, mask_folder_path=train_mask_folder_path, format='image', im_size=image_size)
    val_dataset = UNetDataset(im_folder_path=val_im_folder_path, mask_folder_path=val_mask_folder_path, format='image', im_size=image_size)
    ...
    for epoch in range(epochs):
        ...
        epoch_val_score_list.append(val_dice)

        if save_cp:
            torch.save(net.state_dict(),
                       dir_checkpoint + 'epoch_{}.pt'.format(epoch + 1))
    ...
    with open('epoch_val_score_list.json', 'w') as f:
        json.dump(epoch_val_score_list, f, indent=4)
